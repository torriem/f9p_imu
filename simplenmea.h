 ...
 *
 *  Modified by Michael Torrie for the needs of f9p_imu.  It only breaks out
 *  latitude, longitude, altitude, fix type, and makes a note of where the
 *  parts of the buffer we can ruse are. That we we can just replace the 
 *  latitude, longitude, and altitude fields and leave the rest alone.
 *
...
class SimpleNMEAParser {
public:
	//hack to help us cheaply alter the sentence.
	uint8_t before_latlon;
	uint8_t after_latlon;
	uint8_t before_altitude;
	uint8_t after_altitude;
	...
	SimpleNMEAParser(void);

	// User must decide and allocate the buffer
	SimpleNMEAParser(void* buffer, uint8_t len);
	...
	void setBadChecksumHandler(void (*handler)(const SimpleNMEAParser& nmea)) {
		_badChecksumHandler = handler;
	}

	void setUnknownSentenceHandler(void (*handler)(const SimpleNMEAParser& nmea)) {
...
private:
	...
	char _navSystem;
	bool _isValid;
	uint8_t _fixType; //1=gps,2=dgps,3=pps,4=rtk,5=float rtk
	...
	void (*_badChecksumHandler)(const SimpleNMEAParser &nmea);
	void (*_unknownSentenceHandler)(const SimpleNMEAParser &nmea);
